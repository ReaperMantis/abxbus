...
export class WebSocketRelayEventBridge {      
  ...
  private loop_generation: number  
  ...
  async start(): Promise<void> {      
    ...
    this.start_promise = (async () => {      
      this.running = true  
      const generation = ++this.loop_generation  
      this.ws_connected = withResolvers<void>()      
      this.listener_promise = this.runListenLoop(generation)  
      try {      
        await raceWithConnectTimeout(  
          this.ws_connected.promise,  
          `WebSocketRelayEventBridge to ${this.url}`,  
  ...
  private async runListenLoop(generation: number): Promise<void> {      
    while (this.running && this.loop_generation === generation) {      
      let connected = false      
      try {      
        const ws = await openWebSocket(this.url)      
        if (!this.running || this.loop_generation !== generation) {      
          try { ws.close() } catch { /* ignore */ }      
          return      
        }      
        this.ws = ws      
        ...
        this.ws = null      
        // Reject the old deferred so waiting callers fail fast instead of timing out      
        if (this.running && this.loop_generation === generation && connected) {      
