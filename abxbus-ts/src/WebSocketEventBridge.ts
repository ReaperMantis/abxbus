...
import { importOptionalDependency, isNodeRuntime } from './optional_deps.js'  
...
export class WebSocketEventBridge {  
  ...
  async start(): Promise<void> {  
    ...
    this.start_promise = (async () => {  
      ...
      if (this.send_to) {  
        this.client_connected = createDeferred()  
  ...
  private async clientSend(payload: string): Promise<void> {  
    if (!this.client_ws) {  
      ...
      const timeout_promise = new Promise<never>((_, reject) =>  
        setTimeout(  
          () => reject(new Error(`WebSocketEventBridge: connect timeout for ${this.send_to?.raw}`)),  
          WS_CONNECT_TIMEOUT_MS,  
        ),  
      )  
      await Promise.race([this.client_connected.promise, timeout_promise])  
    }  
  ...
  private serverBroadcast(payload: string): void {  
    ...
    for (const ws of clients) {  
      try { ws.send(payload) } catch { /* ignore individual client errors */ }  
    }  
  }  
  ...
  private async runClientLoop(url: string): Promise<void> {  
    while (this.running) {  
      try {  
        ...
        this.client_ws = null  
        // Reset deferred so future sends will wait for the next reconnect  
        if (this.running) this.client_connected = createDeferred()  
      }  
...
export class WebSocketRelayEventBridge {  
  ...
  async emit<T extends BaseEvent>(event: T): Promise<void> {  
    ...
    if (!this.ws) {  
      ...
      const timeout_promise = new Promise<never>((_, reject) =>  
        setTimeout(  
          () => reject(new Error(`WebSocketRelayEventBridge: connect timeout for ${this.url}`)),  
          WS_CONNECT_TIMEOUT_MS,  
        ),  
      )  
      await Promise.race([this.ws_connected.promise, timeout_promise])  
    }  
  
    const ws = this.ws  
    if (!ws) throw new Error(`WebSocketRelayEventBridge: not connected to ${this.url}`)  
    ws.send(JSON.stringify(event.toJSON()))  
  }  
  ...
  async start(): Promise<void> {  
    ...
    this.start_promise = (async () => {  
      this.ws_connected = createDeferred()  
      this.running = true  
      // Fire-and-forget: loop runs in the background  
  ...
  private async runListenLoop(): Promise<void> {  
    while (this.running) {  
      try {  
        ...
        this.ws = null  
        // Reset deferred so future sends will wait for the next reconnect  
        if (this.running) this.ws_connected = createDeferred()  
      }  
